...
`docs/pilot-wallets.csv` contains 50 deterministic Preprod stress-test addresses generated by `npm run pilot:wallets`. [`docs/pilot-transaction-proof.csv`](./pilot-transaction-proof.csv) mirrors those slots and is the transaction-proof register: `plan_reference` is stable, while `transaction_reference` stays blank and status remains `ADDRESS_READY_TX_PENDING` until the faucet/wallet flow confirms a real hash. This stress-test set is separate from the 50 consented participant rows required below.
