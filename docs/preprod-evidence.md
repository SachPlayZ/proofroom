...
`docs/pilot-wallets.csv` contains 50 deterministic wallet-format fixtures generated by `npm run pilot:wallets`. They are useful for local rehearsal only: every row is marked `NOT_SUBMITTED_TO_PREPROD` and does not satisfy the participant requirement.
